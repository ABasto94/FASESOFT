...
/**
 * Servicios para operaciones CRUD y de negocio sobre la entidad FasRoles
 * 
 * @author GeneradorCRUD
 */
@Stateless
public class NegocioFasRoles extends NegocioAbstracto<FasRoles, FasRolesDTO> {

	@EJB
	private ManejadorFasRoles manejadorFasRoles;

	/**
	 * Variable estatica para imprimir logs...
	 */
	private static final Logger logger = Logger.getLogger(NegocioFasRoles.class.getName());
	 ...
	 *                                   correctamente
	 */
	public List<FasRolesDTO> consultar(String filterBy, String orderBy, Integer from, Integer to)
		...
		return convertirListaEntidadesADao(manejadorFasRoles.consultar(filtros, ordenamiento, rango));
	 ...
	 * @return La insntancia de FasRoles recién creado
	 */
	public FasRolesDTO crear(FasRolesDTO fasRolesDTO) {
		// protected region Modifique el metodo crear on begin

		logService(this.getClass().getName(), "crear", fasRolesDTO);

		FasRoles fasRoles = new FasRoles();
		copiarPropiedades(fasRoles, fasRolesDTO);

		manejadorFasRoles.crear(fasRoles);
	 ...
	 * @return La instancia de la entidad FasRoles que ha sido actualizado
	 */
	public FasRolesDTO actualizar(FasRolesDTO fasRolesDTO) {
		// protected region Modifique el metodo actualizar on begin

		logService(this.getClass().getName(), "actualizar", fasRolesDTO);

		FasRoles fasRoles = manejadorFasRoles.buscar(fasRolesDTO.getIdRol());
		copiarPropiedades(fasRoles, fasRolesDTO);

		manejadorFasRoles.actualizar(fasRoles);
	 ...
	 */
	public String eliminar(Integer idRol) {
		// protected region Modifique el metodo eliminar on begin

		logService(this.getClass().getName(), "eliminar", idRol);
		manejadorFasRoles.eliminarPorId(idRol);

		StringBuilder valores = new StringBuilder();
		valores.append(String.valueOf(idRol));
	...
	public String contar(String filterBy, Integer from, Integer to) throws InvalidParameterException {
		...
		return String.valueOf(manejadorFasRoles.consultarTotalRegistros(filtros, rango));
	...
	public List<String> consultarLista(String filterBy, String orderBy, String atributo)
		...
		return UtilOperaciones.convertirListaObjetosAString(
				manejadorFasRoles.consultarLista(filtros, ordenamiento, infoAgrupamiento));
	...
	protected boolean entidadContieneAtributo(String nombreAtributo) {
		return FasRoles.contieneAtributo(nombreAtributo);
	}
	 ...
	 */
	@Override
	protected FasRolesDTO instanciarDAO() {
		return new FasRolesDTO();
	}

	public int eliminarByProfile(Integer idUsuario) {
		return manejadorFasRoles.eliminarPorUsuario(idUsuario);
	}
