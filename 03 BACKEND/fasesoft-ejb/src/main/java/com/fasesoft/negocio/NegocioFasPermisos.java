...
/**
 * Servicios para operaciones CRUD y de negocio sobre la entidad FasPermisos
 * 
 * @author GeneradorCRUD
 */
@Stateless
public class NegocioFasPermisos extends NegocioAbstracto<FasPermisos, FasPermisosDTO> {

	@EJB
	private ManejadorFasPermisos manejadorFasPermisos;

	/**
	 * Variable estatica para imprimir logs...
	 */
	private static final Logger logger = Logger.getLogger(NegocioFasPermisos.class.getName());
	 ...
	 *                                   correctamente
	 */
	public List<FasPermisosDTO> consultar(String filterBy, String orderBy, Integer from, Integer to)
		...
		return convertirListaEntidadesADao(manejadorFasPermisos.consultar(filtros, ordenamiento, rango));
	 ...
	 * @return La insntancia de FasPermisos recién creado
	 */
	public FasPermisosDTO crear(FasPermisosDTO fasPermisosDTO) {
		// protected region Modifique el metodo crear on begin

		logService(this.getClass().getName(), "crear", fasPermisosDTO);

		FasPermisos fasPermisos = new FasPermisos();
		copiarPropiedades(fasPermisos, fasPermisosDTO);

		manejadorFasPermisos.crear(fasPermisos);

		return fasPermisosDTO;
	 ...
	 *                       JSON.
	 * @return La instancia de la entidad FasPermisos que ha sido actualizado
	 */
	public FasPermisosDTO actualizar(FasPermisosDTO fasPermisosDTO) {
		// protected region Modifique el metodo actualizar on begin

		logService(this.getClass().getName(), "actualizar", fasPermisosDTO);

		FasPermisos fasPermisos = manejadorFasPermisos.buscar(fasPermisosDTO.getIdPermiso());
		copiarPropiedades(fasPermisos, fasPermisosDTO);

		manejadorFasPermisos.actualizar(fasPermisos);
	 ...
	 */
	public String eliminar(Integer idPermiso) {
		// protected region Modifique el metodo eliminar on begin

		logService(this.getClass().getName(), "eliminar", idPermiso);
		manejadorFasPermisos.eliminarPorId(idPermiso);

		StringBuilder valores = new StringBuilder();
		valores.append(String.valueOf(idPermiso));
		return valores.toString();
		// protected region Modifique el metodo eliminar end
	}

	public int eliminarByProfile(Integer idPerfil) {
		return manejadorFasPermisos.eliminarPorPerfil(idPerfil);
	}
	...
	public String contar(String filterBy, Integer from, Integer to) throws InvalidParameterException {
		...
		return String.valueOf(manejadorFasPermisos.consultarTotalRegistros(filtros, rango));
	...
	public List<String> consultarLista(String filterBy, String orderBy, String atributo)
		...
		return UtilOperaciones.convertirListaObjetosAString(
				manejadorFasPermisos.consultarLista(filtros, ordenamiento, infoAgrupamiento));
	...
	protected boolean entidadContieneAtributo(String nombreAtributo) {
		return FasPermisos.contieneAtributo(nombreAtributo);
	}
	 ...
	 */
	@Override
	protected FasPermisosDTO instanciarDAO() {
		return new FasPermisosDTO();
	}
