...
import static com.fasesoft.modelo.entidades.FasAfiliados.ENTIDAD_FAS_AFILIADOS_NOMBRE;
import static com.fasesoft.modelo.entidades.FasAfiliados.ENTIDAD_FAS_AFILIADOS_APELLIDO;

import com.fasesoft.modelo.entidades.FasAfiliados;

import static com.fasesoft.modelo.entidades.FasAfiliados.ENTIDAD_FAS_AFILIADOS_PK;
...
/**
 * Pruebas de los métodos expuestos por el ManejadorFasAfiliados
 *
 * @author GeneradorCRUD
 */
@RunWith(Arquillian.class)
public class ManejadorFasAfiliadosTest {

    public static final String DEPLOY = "Prueba"; 
    //Debe ser mayor o igual a 2
    private static final int REGISTROS_A_CREAR = 10;
    
    private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(ManejadorFasAfiliadosTest.class.getName());
    ...
    @Inject
    private ManejadorFasAfiliados manejador;
    ...
    //Lista de instancias de FasAfiliados generadas aleatoriamente por PODAM
    private final List<FasAfiliados> data = new ArrayList<>();   
    ...
    public static JavaArchive createDeployment() {
        ...
        return ShrinkWrap.create(JavaArchive.class, DEPLOY + ".jar")
                .addPackage(ManejadorFasAfiliados.class.getPackage())
                .addPackage(ManejadorPersistencia.class.getPackage())
                .addPackage(FasAfiliados.class.getPackage())
    ...
    public void configTest() throws Exception{
        logger.debug("Configurando Test " + ManejadorFasAfiliadosTest.class.getName());
        clearData();
        insertData();
        startTransaction();
    }
    ...
    public void buscarObjectTest() {
        FasAfiliados entidad = data.get(0);
        FasAfiliados entidadConsultada = manejador.buscar(entidad.getCorreo());
    ...
    public void eliminarTest() {
        ...
        FasAfiliados deleted = em.find(FasAfiliados.class, entidad.getCorreo());
    ...
    public void eliminarPorIdTest() {
        FasAfiliados entidad = data.get(0);
        manejador.eliminarPorId(entidad.getCorreo());
        FasAfiliados deleted = em.find(FasAfiliados.class,entidad.getCorreo());
        Assert.assertNull(deleted);        
    }
    ...
    public void eliminarPorFiltroTest() {
        ...
        filtros.add(new InformacionFiltro(TipoFiltro.EXACTO, ENTIDAD_FAS_AFILIADOS_PK, entidad.getCorreo(), ENTIDAD_FAS_AFILIADOS_PK));
    ...
    public void crearTest() {     
        FasAfiliados entidad = generarFasAfiliados();
        manejador.crear(entidad);
        FasAfiliados creada = em.find(FasAfiliados.class, entidad.getCorreo());        
    ...
    public void actualizarTest() {
        FasAfiliados entidad = data.get(0);
        String idEntidad = entidad.getCorreo();
        FasAfiliados entidad2 = data.get(1);
        copiarPropiedades(entidad, entidad2);
        entidad.setCorreo(idEntidad);
        manejador.actualizar(entidad);
        FasAfiliados entidadDB = em.find(FasAfiliados.class, idEntidad);
    ...
    public void actualizarPorFiltrosTest() {
        ...
        asignaciones.add(new InformacionAsignacion(ENTIDAD_FAS_AFILIADOS_NOMBRE, entidad2.getNombre()));
        
        manejador.actualizarPorFiltros(asignaciones, filtros);        
        
        FasAfiliados entidadActualizada = em.find(FasAfiliados.class, entidad.getCorreo());
    ...
    public void consultarTodosTest() {
        List<FasAfiliados> listaConsulta = manejador.consultar(null, null,null);

        Assert.assertEquals(listaConsulta.size(), data.size());
        for(FasAfiliados instancia : listaConsulta){
    ...
    public void consultarConFiltroPKTest() {
        ...
        List<InformacionFiltro> filtros = new ArrayList<>();
        filtros.add(new InformacionFiltro(TipoFiltro.EXACTO, ENTIDAD_FAS_AFILIADOS_PK, entidad.getCorreo(), ENTIDAD_FAS_AFILIADOS_PK));
        List<FasAfiliados> listaConsulta = manejador.consultar(filtros, null,null);
    ...
    public void consultarConDosFiltrosTest() {        
	        ...
	        List<InformacionFiltro> filtros = new ArrayList<>();
	        filtros.add(new InformacionFiltro(TipoFiltro.EXACTO, ENTIDAD_FAS_AFILIADOS_NOMBRE, entidad.getNombre(), ENTIDAD_FAS_AFILIADOS_NOMBRE));
			filtros.add(new InformacionFiltro(TipoFiltro.EXACTO, ENTIDAD_FAS_AFILIADOS_APELLIDO, entidad.getApellido(), ENTIDAD_FAS_AFILIADOS_APELLIDO));
    ...
    public void consultarConOrdenamientoTest() {		
        List<FasAfiliados> listaOrdenada = data;
        Collections.sort(listaOrdenada, new NombreComparator());
                
        List<InformacionOrdenamiento> ordenamiento = new ArrayList<>();
        ordenamiento.add(new InformacionOrdenamiento(TipoOrdenamiento.ASCENDENTE, ENTIDAD_FAS_AFILIADOS_NOMBRE));
        
        List<FasAfiliados> listaConsulta = manejador.consultar(null, ordenamiento,null);
    ...
    public void consultarConRangoTest() {
        ...
        List<FasAfiliados> listaConsulta = manejador.consultar(null, null,rango);
    ...
    public void consultarListaTest(){
        InformacionAgrupamiento infoAgrupamiento = new InformacionAgrupamiento(FuncionAgrupamientoJPQL.DISTINCT, ENTIDAD_FAS_AFILIADOS_NOMBRE);
    ...
    private void insertData()  {        
        try {            
            ...
            for (int i = 0; i < REGISTROS_A_CREAR; i++) {            
                FasAfiliados entity = generarFasAfiliados();                       
                //Persiste el objeto en base de datos
                em.persist(entity);
                //Se añade a la lista del oráculo
                data.add(entity);
                logger.debug("Se inserto FasAfiliados: " + entity.toString());
            }
            utx.commit();
    ...
    /**
     * Genera una nueva instancia de FasAfiliados cuyo id no se encuentre ya en la base de datos.
     * 
     * @param factory La fábrica podam para generar POJOS
     * @return Una instancia de categoría
     */
    private FasAfiliados generarFasAfiliados(){
        FasAfiliados entity = null;
        while(entity==null){
            entity = factory.manufacturePojo(FasAfiliados.class);
            // protected region atributos adicionales on begin
            // Escriba en esta sección sus modificaciones
            ...
            if( em.find(FasAfiliados.class, entity.getCorreo())!=null ){
                entity = null;
     ...
     * la entidad FasAfiliados, también elimina la lista correspondiente a estos
    ...
    private void clearData() throws TestException {        
        try {
            utx.begin();
            em.joinTransaction();
            em.createQuery("DELETE FROM FasAfiliados").executeUpdate();
            data.clear();
            utx.commit();
    ...
    private static class NombreComparator implements Comparator<FasAfiliados> {
    
        @Override
        public int compare(FasAfiliados instancia1, FasAfiliados instancia2) {
