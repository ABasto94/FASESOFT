...
import static com.fasesoft.modelo.entidades.FasAportes.ENTIDAD_FAS_APORTES_BENEFICIO;
import static com.fasesoft.modelo.entidades.FasAportes.ENTIDAD_FAS_APORTES_APORTE;
import java.math.BigDecimal;
import com.fasesoft.modelo.entidades.FasAportes;
import java.math.BigDecimal;
import static com.fasesoft.modelo.entidades.FasAportes.ENTIDAD_FAS_APORTES_PK;
...
import com.fasesoft.modelo.manejadores.ManejadorFasAportes;
...
/**
 * Pruebas de los métodos expuestos por el ManejadorFasAportes
 *
 * @author GeneradorCRUD
 */
@RunWith(Arquillian.class)
public class ManejadorFasAportesTest {

    public static final String DEPLOY = "Prueba"; 
    //Debe ser mayor o igual a 2
    private static final int REGISTROS_A_CREAR = 10;
    
    private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(ManejadorFasAportesTest.class.getName());
    ...
    private static final BigDecimal[] VALORES_BENEFICIO = {new BigDecimal(111.11d), new BigDecimal(222.22d), new BigDecimal(333.33d)};
    
    @Inject
    private ManejadorFasAportes manejador;
    ...
    //Lista de instancias de FasAportes generadas aleatoriamente por PODAM
    private final List<FasAportes> data = new ArrayList<>();   
    ...
    public static JavaArchive createDeployment() {
        ...
        return ShrinkWrap.create(JavaArchive.class, DEPLOY + ".jar")
                .addPackage(ManejadorFasAportes.class.getPackage())
                .addPackage(ManejadorPersistencia.class.getPackage())
                .addPackage(FasAportes.class.getPackage())
    ...
    public void configTest() throws Exception{
        logger.debug("Configurando Test " + ManejadorFasAportesTest.class.getName());
        clearData();
        insertData();
        startTransaction();
    }
    ...
    public void buscarObjectTest() {
        FasAportes entidad = data.get(0);
        FasAportes entidadConsultada = manejador.buscar(entidad.getIdAporte());
    ...
    public void eliminarTest() {
        ...
        FasAportes deleted = em.find(FasAportes.class, entidad.getIdAporte());
    ...
    public void eliminarPorIdTest() {
        FasAportes entidad = data.get(0);
        manejador.eliminarPorId(entidad.getIdAporte());
        FasAportes deleted = em.find(FasAportes.class,entidad.getIdAporte());
        Assert.assertNull(deleted);        
    }
    ...
    public void eliminarPorFiltroTest() {
        ...
        filtros.add(new InformacionFiltro(TipoFiltro.EXACTO, ENTIDAD_FAS_APORTES_PK, entidad.getIdAporte(), ENTIDAD_FAS_APORTES_PK));
    ...
    public void crearTest() {     
        FasAportes entidad = generarFasAportes();
        manejador.crear(entidad);
        FasAportes creada = em.find(FasAportes.class, entidad.getIdAporte());        
    ...
    public void actualizarTest() {
        FasAportes entidad = data.get(0);
        BigDecimal idEntidad = entidad.getIdAporte();
        FasAportes entidad2 = data.get(1);
        copiarPropiedades(entidad, entidad2);
        entidad.setIdAporte(idEntidad);
        manejador.actualizar(entidad);
        FasAportes entidadDB = em.find(FasAportes.class, idEntidad);
    ...
    public void actualizarPorFiltrosTest() {
        ...
        asignaciones.add(new InformacionAsignacion(ENTIDAD_FAS_APORTES_BENEFICIO, entidad2.getBeneficio()));
        
        manejador.actualizarPorFiltros(asignaciones, filtros);        
        
        FasAportes entidadActualizada = em.find(FasAportes.class, entidad.getIdAporte());
        
        Assert.assertEquals(entidadActualizada.getBeneficio(),entidad2.getBeneficio());
    }
    
    @Test
    public void consultarTodosTest() {
        List<FasAportes> listaConsulta = manejador.consultar(null, null,null);

        Assert.assertEquals(listaConsulta.size(), data.size());
        for(FasAportes instancia : listaConsulta){
    ...
    public void consultarConFiltroPKTest() {
        ...
        List<InformacionFiltro> filtros = new ArrayList<>();
        filtros.add(new InformacionFiltro(TipoFiltro.EXACTO, ENTIDAD_FAS_APORTES_PK, entidad.getIdAporte(), ENTIDAD_FAS_APORTES_PK));
        List<FasAportes> listaConsulta = manejador.consultar(filtros, null,null);
    ...
    public void consultarConDosFiltrosTest() {        
	        ...
	        List<InformacionFiltro> filtros = new ArrayList<>();
	        filtros.add(new InformacionFiltro(TipoFiltro.EXACTO, ENTIDAD_FAS_APORTES_BENEFICIO, entidad.getBeneficio(), ENTIDAD_FAS_APORTES_BENEFICIO));
			filtros.add(new InformacionFiltro(TipoFiltro.EXACTO, ENTIDAD_FAS_APORTES_APORTE, entidad.getAporte(), ENTIDAD_FAS_APORTES_APORTE));
    ...
    public void consultarConOrdenamientoTest() {		
        List<FasAportes> listaOrdenada = data;
        Collections.sort(listaOrdenada, new BeneficioComparator());
                
        List<InformacionOrdenamiento> ordenamiento = new ArrayList<>();
        ordenamiento.add(new InformacionOrdenamiento(TipoOrdenamiento.ASCENDENTE, ENTIDAD_FAS_APORTES_BENEFICIO));
        
        List<FasAportes> listaConsulta = manejador.consultar(null, ordenamiento,null);
    ...
    public void consultarConRangoTest() {
        ...
        List<FasAportes> listaConsulta = manejador.consultar(null, null,rango);
    ...
    public void consultarListaTest(){
        InformacionAgrupamiento infoAgrupamiento = new InformacionAgrupamiento(FuncionAgrupamientoJPQL.DISTINCT, ENTIDAD_FAS_APORTES_BENEFICIO);
        
        List<String> lista = convertirListaObjetosAString(manejador.consultarLista(null, null, infoAgrupamiento));
        
        Assert.assertTrue(lista.size()<=VALORES_BENEFICIO.length);
        Assert.assertTrue(lista.size()>0);
        for(String valor : lista){
            Assert.assertTrue(Arrays.asList(VALORES_BENEFICIO).contains(valor));
        }
    ...
    private void insertData()  {        
        try {            
            ...
            for (int i = 0; i < REGISTROS_A_CREAR; i++) {            
                FasAportes entity = generarFasAportes();                       
                //Persiste el objeto en base de datos
                em.persist(entity);
                //Se añade a la lista del oráculo
                data.add(entity);
                logger.debug("Se inserto FasAportes: " + entity.toString());
            }
            utx.commit();
    ...
    /**
     * Genera una nueva instancia de FasAportes cuyo id no se encuentre ya en la base de datos.
     * 
     * @param factory La fábrica podam para generar POJOS
     * @return Una instancia de categoría
     */
    private FasAportes generarFasAportes(){
        FasAportes entity = null;
        while(entity==null){
            entity = factory.manufacturePojo(FasAportes.class);
            // protected region atributos adicionales on begin
            // Escriba en esta sección sus modificaciones

            // protected region atributos adicionales end
            //Este valor se setea para el test de agrupamiento
            entity.setBeneficio(obtenerBeneficioAleatoria());
            if( em.find(FasAportes.class, entity.getIdAporte())!=null ){
                entity = null;
     ...
     * la entidad FasAportes, también elimina la lista correspondiente a estos
    ...
    private void clearData() throws TestException {        
        try {
            utx.begin();
            em.joinTransaction();
            em.createQuery("DELETE FROM FasAportes").executeUpdate();
            data.clear();
            utx.commit();
    ...
    private static class BeneficioComparator implements Comparator<FasAportes> {
    
        @Override
        public int compare(FasAportes instancia1, FasAportes instancia2) {
            return instancia1.getBeneficio().compareTo(instancia2.getBeneficio());
        }
    }
    
    private static BigDecimal obtenerBeneficioAleatoria(){
        Random generator = new Random();
        int index = generator.nextInt(VALORES_BENEFICIO.length);
        
        return VALORES_BENEFICIO[index];
    }
