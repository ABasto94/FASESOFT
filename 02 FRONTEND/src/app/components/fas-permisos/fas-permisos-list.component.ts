...
@Component({
  selector: 'fas-permisos-list',
  templateUrl: './fas-permisos-list.component.html',
  providers:  [UriProvider, GenericClientService]
})
export class FasPermisosListComponent implements OnInit {

  private sub: any;
  private fasPermisosList : FasPermisos[];
  private queryParamList: [string, string][] = [['','']];

  private metadata : FasPermisosMetadata;
  private fasPermisosDomain : FasPermisosDomain = new FasPermisosDomain();
  private fieldList: Field[];
  private modalDeleteMessage : ConfirmationModalOptions;
  private openGenericSearch : boolean = false;
  private localAlert : AlertObject = new AlertObject(null, null, null, 0);
	
  private selectedFasPermisos: FasPermisos;
  constructor(
  ...
  ngOnInit(): void {
  	this.fieldList = this.fasPermisosDomain.getSearchListOfFields();
    this.metadata = this.fasPermisosDomain.getFasPermisosMetadata();
	this.getFasPermisosList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getFasPermisosList(this.queryParamList);
     }
   	 } else {
       this.getFasPermisosList(null);
     }
   ...
   onFasPermisosDelete(fasPermisos : FasPermisos) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['idPermiso', fasPermisos.idPermiso.toString()]);
    this.deleteFasPermisos(queryParam);
  }
  
  onSelectedFasPermisos(fasPermisos: FasPermisos){
    this.selectedFasPermisos = fasPermisos;
  }
   
   getFasPermisosList(queryParamList: [string, string][]) {
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.fasPermisosList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteFasPermisos(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'FasPermisos';
    let pathParams : Array<[number, string]> = null;
  ...
  getConfirmationResponse($event : [boolean, any]) {
    if ($event[0]) {
      ...
      this.onFasPermisosDelete($event[1]);
    }
  }
