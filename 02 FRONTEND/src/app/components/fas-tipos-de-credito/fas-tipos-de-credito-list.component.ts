...
@Component({
  selector: 'fas-tipos-de-credito-list',
  templateUrl: './fas-tipos-de-credito-list.component.html',
  providers:  [UriProvider, GenericClientService]
})
export class FasTiposDeCreditoListComponent implements OnInit {

  private sub: any;
  private fasTiposDeCreditoList : FasTiposDeCredito[];
  private queryParamList: [string, string][] = [['','']];

  private metadata : FasTiposDeCreditoMetadata;
  private fasTiposDeCreditoDomain : FasTiposDeCreditoDomain = new FasTiposDeCreditoDomain();
  private fieldList: Field[];
  private modalDeleteMessage : ConfirmationModalOptions;
  private openGenericSearch : boolean = false;
  private localAlert : AlertObject = new AlertObject(null, null, null, 0);
	
  private selectedFasTiposDeCredito: FasTiposDeCredito;
  constructor(
  ...
  ngOnInit(): void {
  	this.fieldList = this.fasTiposDeCreditoDomain.getSearchListOfFields();
    this.metadata = this.fasTiposDeCreditoDomain.getFasTiposDeCreditoMetadata();
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getFasTiposDeCreditoList(this.queryParamList);
     }
   	 } else {
       this.getFasTiposDeCreditoList(null);
     }
   ...
   onFasTiposDeCreditoDelete(fasTiposDeCredito : FasTiposDeCredito) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['idTipo', fasTiposDeCredito.idTipo.toString()]);
    this.deleteFasTiposDeCredito(queryParam);
  }
  
  onSelectedFasTiposDeCredito(fasTiposDeCredito: FasTiposDeCredito){
    this.selectedFasTiposDeCredito = fasTiposDeCredito;
  }
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.fasTiposDeCreditoList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteFasTiposDeCredito(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'FasTiposDeCredito';
    let pathParams : Array<[number, string]> = null;
  ...
  getConfirmationResponse($event : [boolean, any]) {
    if ($event[0]) {
      ...
      this.onFasTiposDeCreditoDelete($event[1]);
    }
  }
