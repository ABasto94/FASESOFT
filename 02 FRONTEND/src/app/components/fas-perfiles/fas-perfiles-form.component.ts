...
@Component({
  selector: 'fas-perfiles-form',
  templateUrl: './fas-perfiles-form.component.html',
  providers: [UriProvider, GenericClientService, ]
})
export class FasPerfilesFormComponent implements AfterViewChecked {
  ...
  onSubmit() {
    this.submitted = true;
    this._spinnerService.show();
    if (this.isNewFasPerfiles) {
      this.postFasPerfiles(this.fasPerfiles);
    } else {
      this.put(this.fasPerfiles);
    }
    if (this.padre) {
        ...
        this.genericClientService.post(uriService, request).subscribe(result => {
          ...
          for (let i = 0; i < this.fasAccesosList.length; i++) {
            ...
            if(this.fasAccesosList[i].elegido){
              ...
              this.postFasPermisos(permiso,this.fasAccesosList[i]);
            }          
          }
  ...
  postFasPermisos(request: any, acceso:FasAccesos) {
    let response: any;
  ...
  volver(){
    window.scroll(0,0);
    this.volverClick = !this.volverClick;
  }
